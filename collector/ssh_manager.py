...
class OntMonitor:
    ...
    def run_command(self, command, wait=3):
        """Runs a command and returns the cleaned output."""
        if not self.shell:
            raise Exception("SSH shell not connected.")
        ...
        cleaned_output = cleaned_output.replace('\x07', '')
        return cleaned_output

def load_ssh_config(host_alias):
    """Loads SSH connection details from ~/.ssh/config."""
